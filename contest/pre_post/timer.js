...
function checkTime(compID, contestType){
	...
	var time = $.ajax({
		url: "http://localhost/middleware/timer2/time.php", // The url to the time.php
		method: "POST",
		data: {contestID: compID}
	});
	
	time.done(function(response){
		dateDiff(response, contestType);
	});
}

/* This function edits the front-end in terms of what message is displayed and the 
	appropiate time. */
function dateDiff(response, contestType){
	...
	switch(diff['signal']){
		case 1: // Time before the competition 
			...
			if(contestType == 'on-time'){
				location.replace('http://localhost/middleware/timer2/front.php');
			} else {
				preContest(diff);
			}
			break;
		...
		case 0:
			if(contestType == 'pre'){
				location.replace('http://localhost/middleware/competition/front.php');
...
/*
// Function to create the button to start the competition.
function createButton(){
	var button = document.createElement('button');
		button.id = "compButton";
		button.onclick = function(){
			var url = "http://www.google.com";
			window.location = url;
		};
		button.style = "margin-top: 3%; margin-left:42%;";
		button.innerHTML = "Start Competition";
