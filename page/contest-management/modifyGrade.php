<?php
	...
	if(isset($_POST['submissionID'])){
		...
		// Check if the grade is different than the posted value.
		$team_sub = Submission :: get_submission($_POST['submissionID']);
		$answers = Question :: get_answers($team_sub[0]['question_FK']);
		$data = 1;
		foreach($answers as $ans){
			$team_grade = Submission::get_answer($_POST['teamID'], $ans['qio_PK']);
			$data = $data * $team_grade['grade'];
		}
		
		if($data == $_POST['correct']){
			$msg = "Update cancelled since there is no change!";
			...
			if($_POST['correct'] == 1)
				$updateScore = Grade :: update_score($_POST['contestID'], $_POST['teamID'], $getScore + 1);
			else
				$updateScore = Grade :: update_score($_POST['contestID'], $_POST['teamID'], $getScore - 1);
				
			if($updateGrade && $updateScore)
		...
		if(!isset($_GET['submission'])){
			...
			echo '<h4><div style="white-space: pre-wrap;">'.$ques['qtext'].'</div></h4>';
			...
			echo '<div style="white-space: pre-wrap; background-color: transparent; color: black; font-family: monospace;">';
				$find = array('<', '>');
				$rep = array('&lt;', '&gt;');
